...
from (
        ...
        from (
                select productCode,
                    match(productName) against('davidson bar' IN NATURAL LANGUAGE MODE) as score
                from products
            ) as a,
            (
                select productCode,
                    match(productDescription) against('davidson bar' IN NATURAL LANGUAGE MODE) as score
                from products
            ) as b
        where a.productCode = b.productCode
    ) as c
